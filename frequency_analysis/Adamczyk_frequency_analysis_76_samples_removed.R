...
# Purpose: ASV frequency analysis - using RANK 6 non-rarefied ASVs and 76 SAMPLES REMOVED. The read threshold is >= 10
...
meta_otu <- read.csv("seagrass_transplant_16s_meta_otu_NOT_RAREFIED_RANK_6_76removed.csv", header = T)

# reading in taxa table
taxa <- read.csv("seagrass_transplant_16s_NOT_RAREFIED_RANK_6_76removed.tax.csv", header=T)
...
#####################################
# TABLE S8 - Supplementary analyses #
#####################################
...
for(c in 13:540) {
  for(r in 1:70) {
    natural_2[71,c] <-  natural_2[71,c] + natural_2[r,c]
  }
  natural_2[71,c] <-  natural_2[71,c]/70
}


# saving row 95 as its own df
natural_results <- natural_2[71, 13:540]
...
for(c in 13:540) {
  for(r in 1:35) {
    asu_2[36,c] <-  asu_2[36,c] + asu_2[r,c]
  }
  asu_2[36,c] <-  asu_2[36,c]/35
}


# saving row 47 asu its own df
asu_results <- asu_2[36, 13:540]
...
sterile_final_2[44,] <- 0

# running for loop to calculate frequency
# Thank you Martin!!!
for(c in 13:540) {
  for(r in 1:43) {
    sterile_final_2[44,c] <-  sterile_final_2[44,c] + sterile_final_2[r,c]
  }
  sterile_final_2[44,c] <-  sterile_final_2[44,c]/43
}


# saving row 44 sterile_final its own df
sterile_final_results <- sterile_final_2[44, 13:540]
...
for(c in 13:540) {
  for(r in 1:16) {
    water_2[17,c] <-  water_2[17,c] + water_2[r,c]
  }
  water_2[17,c] <-  water_2[17,c]/16
}


# saving row 17 water its own df
water_results <- water_2[17, 13:540]

# transposing water_results row so that it is a single column
water_results2 <- as.data.frame(t(water_results))
names(water_results2)[1] <- "water_frequency" # renaming 17 to frequency
...
# saving df as CSV
write.csv(all_treatments.2, "frequency_table_76removed.csv") # this is Table S8 in the supplementary analyses document
