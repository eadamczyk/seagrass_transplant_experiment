...
# Purpose: PERMANOVAs with 76 samples removed
...
meta_otu <- read.csv("metadata_PERMANOVA_76removed_20220427.csv", header=T)
...
### Create table containing only ASVs
bact_abund_asu_sterile_control <- asu_sterile_control %>% select(-c(1:12, 1687:1690))
...
### Create table containing only ASVs
bact_abund_asu_sterile_control_all_sites <- asu_sterile_control_all_sites %>% select(-c(1:12, 1687:1690))
...
### Create table containing only ASVs
bact_abund_asu_sterile_control_pruth <- asu_sterile_control_pruth %>% select(-c(1:12, 1687:1690))
...
### Create table containing only ASVs
bact_abund_asu_sterile_control_choked <- asu_sterile_control_choked %>% select(-c(1:12, 1687:1690))
...
### Create table containing only ASVs
bact_abund_asu_sterile_control <- asu_sterile_control_x %>% select(-c(1:12, 1687:1690))
...
# filtering ASVs only
bact_abund_pruth_region <- pruth_region %>% select(-c(1:12, 1687:1690))
...
bact_abund_choked_region <- choked_region %>% select(-c(1:12, 1687:1690))
